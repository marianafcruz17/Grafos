...
struct grafo{
    ...
    int** arestas;
    float** pesos;
    //define quantas arestas o grafo já possui
    int* grau;
};
...
int main(){
    ...
    insere_aresta(gr,1,3,digrafo,0);
    insere_aresta(gr,1,2,digrafo,0);
    insere_aresta(gr,2,4,digrafo,0);
    insere_aresta(gr,3,0,digrafo,0);
...
Grafo* cria_grafo(int num_vertices,int grau_max,int ponderado){
    ...
    if(gr!=NULL){
        ...
        //garante se a informação é 0 ou 1
...
int insere_aresta(Grafo *gr,int orig,int dest,int digrafo,float peso){
    ...
    //verifica a existencia de verices
    ...
    if(digrafo==0)
        //chama de forma recursiva, alterando a orig e dest
...
void busca_profundidade(Grafo *gr,int ini,int *visitado,int cont){
    ...
    //marca o vertice visitado e visita os vizinhos ainda não visitados
    visitado[ini] = cont;
...
int procura_menor_distancia(float *dist,int *visitado,int num_vertice){
    ...
    for(i=0;i<num_vertice;i++){
        //procura vertice com menor distancia e que nao foi visitado
...
void menor_caminho(Grafo *gr,int ini,int *ant,float *dist){
    ...
    for(i=0;i<gr->grau[u];i++){
        ...
        //atualiza distancias dos visinhos
        if(dist[ind]<0){
            dist[ind] = dist[u] + 1;
            ant[ind] = u;
        }else{
